...
RUN --mount=type=cache,target=/go/pkg/mod \
    ...
    -o /out/defermq-pusher ./cmd/defermq-pusher

FROM alpine:3.23
RUN apk add --no-cache ca-certificates tzdata \
    && addgroup -S -g 10001 defermq \
    && adduser -S -D -H -u 10001 -G defermq defermq
COPY --from=builder --chown=defermq:defermq /out/defermq-pusher /usr/local/bin/defermq-pusher
USER defermq:defermq
EXPOSE 8082
ENTRYPOINT ["/usr/local/bin/defermq-pusher"]
