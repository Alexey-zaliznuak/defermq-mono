...
-- +goose Down
DROP INDEX IF EXISTS deliveries_hot_repair_idx;
ALTER TABLE deliveries
    DROP CONSTRAINT IF EXISTS deliveries_ready_published_pair_check,
    DROP CONSTRAINT IF EXISTS deliveries_ready_revision_check,
    DROP COLUMN IF EXISTS ready_published_at,
    DROP COLUMN IF EXISTS ready_published_revision;
ALTER TYPE nats_outbox_kind RENAME VALUE 'hot_register' TO 'schedule';
